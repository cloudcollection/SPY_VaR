...
SECTION5_FIGURE_FALLBACK = (
    PROJECT_ROOT.parent
    / "MiniProject"
    / "section_05_neural_quantile_regression"
    / "outputs"
    / "figures"
)
...
SECTION_SOURCES = [
    REPORT_DIR / "Demo_Section1.md",
    REPORT_DIR / "section_2_data_features_var_backtesting_framework_en.md",
    REPORT_DIR / "section_3_historical_simulation_final.md",
    REPORT_DIR / "section_4_garch_t_final.md",
    REPORT_DIR / "section_5_neural_quantile_regression.md",
    REPORT_DIR / "Demo_Section6.md",
]
...
def rewrite_image_paths(text: str, source_md: Path) -> str:
    def replace(match: re.Match[str]) -> str:
        ...
        if not image_path.exists():
            fallback_path = SECTION5_FIGURE_FALLBACK / Path(raw_path).name
...
def load_exporter():
    ...
    sys.path.insert(0, str((PROJECT_ROOT / "scripts").resolve()))
...
def build_submission_package() -> None:
    ...
    for dirname in ["scripts", "data", "report"]:
    ...
    external_scripts = [
        PROJECT_ROOT.parent / "05_neural_quantile_var.py",
        PROJECT_ROOT.parent / "05c_garch_anchored_neural_var.py",
        PROJECT_ROOT.parent / "05d_tune_model_c_hyperparams.py",
    ]
    ...
    copytree_clean(PROJECT_ROOT.parent / "results", code_dir / "external_section5_results")
